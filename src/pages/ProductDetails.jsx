...
import { FaStar, FaStarHalfAlt, FaHeart, FaShareAlt, FaMinus, FaPlus, FaCheckCircle } from 'react-icons/fa';
            ...
            <div className="max-w-7xl mx-auto">
                                ...
                                {allImages.length > 1 && (
                                    <>
                                        <button 
                                            ...
                                            className="absolute left-2 top-1/2 -translate-y-1/2 bg-white/80 hover:bg-white p-2 rounded-full shadow-md text-primary transition-all cursor-pointer"
                                        >
                                            <ArrowLeft size={20} />
                                        </button>
                                        <button 
                                            onClick={() => setActiveImage((prev) => (prev === allImages.length - 1 ? 0 : prev + 1))}
                                            className="absolute right-2 top-1/2 -translate-y-1/2 bg-white/80 hover:bg-white p-2 rounded-full shadow-md text-primary transition-all rotate-180 cursor-pointer"
                                        >
                                            <ArrowLeft size={20} />
                                        </button>
                                    </>
                            ...
                            <div className="flex gap-4 overflow-x-auto pb-2 scrollbar-hide">
                                {allImages.map((img, index) => (
                                    <button 
                                        ...
                                        className={`relative w-24 h-24 flex-shrink-0 rounded-xl overflow-hidden border-2 transition-all cursor-pointer ${activeImage === index ? 'border-primary shadow-md scale-105' : 'border-transparent hover:border-gray-300'}`}
                                    >
                        ...
                        <div className="lg:w-1/2 p-6 md:p-10 flex flex-col">
                            ...
                            <div className="flex flex-wrap gap-3 mb-6">
                                {product.brand && (
                                    <div className="flex items-center gap-2 bg-primary/10 text-primary px-3 py-1.5 rounded-full border border-primary/20">
                                        {product.brand.logo && (
                                            <img src={product.brand.logo} alt={product.brand.name} className="w-5 h-5 object-contain rounded-full bg-white p-0.5" />
                                        )}
                                        <span className="text-xs font-bold uppercase tracking-wider">{product.brand.name}</span>
                                    </div>
                                )}
                                {product.category && (
                                    <div className="flex items-center gap-2 bg-secondary/10 text-secondary px-3 py-1.5 rounded-full border border-secondary/20">
                                        {product.category.logo && (
                                            <img src={product.category.logo} alt={product.category.name} className="w-5 h-5 object-contain rounded-full bg-white p-0.5" />
                                        )}
                                        <span className="text-xs font-bold uppercase tracking-wider">{product.category.name}</span>
                                    </div>
                                )}
                                ...
                                <div className="flex items-center gap-1 text-primary">
                                    {[...Array(5)].map((_, i) => (
                                        <FaStar 
                                            key={i} 
                                            size={18} 
                                            className={i < Math.floor(product.rating || 0) ? "text-primary" : "text-gray-300"}
                                        />
                                    ))}
                                    <span className="ml-1 font-bold text-gray-900">{product.rating || 0}</span>
                                </div>
                            ...
                            <div className="flex items-center gap-4 mb-8 bg-gray-50 p-6 rounded-2xl border border-gray-100">
                                <div className="flex flex-col">
                                    {hasDiscount && (
                                        <span className="text-gray-400 text-sm font-medium line-through">৳{price}</span>
                                    )}
                                    <span className="text-4xl font-black text-primary">৳{hasDiscount ? discountPrice : price}</span>
                                </div>
                                {hasDiscount && (
                            ...
                            <div className="mt-auto space-y-6">
                                <div className="flex flex-col sm:flex-row items-center gap-6">
                                    ...
                                    <button 
                                        ...
                                        className="flex-grow w-full sm:w-auto bg-primary text-white h-14 rounded-2xl flex items-center justify-center gap-3 font-bold text-lg hover:bg-primary/90 transition-all active:scale-95 shadow-xl shadow-primary/20 cursor-pointer group"
                                    >
                                ...
                                <div className="flex items-center justify-center sm:justify-start gap-6 pt-4 border-t border-gray-100">
                                    ...
                                    <p className="text-gray-400 text-sm">Free shipping across Bangladesh on orders over ৳5000</p>
                                </div>
                            </div>
                        </div>
                    </div>
                ...
                {product.description && (
                    <div className="mt-12 bg-white rounded-3xl p-8 shadow-lg border border-gray-100">
                        <h2 className="text-2xl font-bold text-gray-900 mb-6 pb-2 border-b-2 border-primary w-fit">Product Description</h2>
                        <div 
